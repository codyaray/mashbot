...
\perlnewcommand{\services}[2]{
	...
	foreach $service (@serviceNames) {
		...
		$returning = $returning .  $serviceArticles[0].' valid \\\\';
...
\begin{hangparas}{0.25in}{1}
	...
	\Feature{#1 #2}{#1 a #2 #6}{client}{#8}

	\Scenario{#1ing a #2 #7 #3}{Twitter}{ }{Twitter}{ }
	\Scenario{#1ing a #2 #7 #3}{Twitter,Facebook}{ }{Twitter}{ } 

Scenario: #1ing a #2 #7 #3 
	Given that I have sent a request 
	And the authentication credentials are valid 
	#5 
	...
	Then there will be a #2 posted on behalf of the user whose credentials were 
	provided to #3 

Scenario: #1ing a #2 #7 #3 and #4 
	Given that I have sent a request 
	And the authentication credentials are valid 
	#5 
	And I have provided authentication credentials for #3 
	...
	Then there will be a #2 #1ed #7 services on behalf of the user whose 
	credentials were provided to #3 and #4 
...
reject our request. 
	...
	And Those credentials are valid for #4 
	And I have indicated an association with the service #4 
	Then there will be an error returned to the user indicating which request
	succeeded and which requests failed. 
\end{hangparas}
}
...
\subsubsection{Feature: Push Status}
	\CoreTestCases{Push}{Status}{Twitter}{Facebook}{And I have filled in at least the status field in the object model}{And I have not filled in at least the status field in the object model}{to}{Hello} 
