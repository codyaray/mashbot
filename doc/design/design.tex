...
\begin{center}
  \includegraphics[scale=0.75]{dot/context.png}
\end{center}
...
\begin{itemize}
	\item Java. The team has a broad base of experience with Java and some team 
	members have experience developing an enterprise application on the J2EE.  
	Additionally, the frameworks available are very robust and nice for building 
	web services on top of.
...
completely configurable in XML.
	\item Plugin Framework/Manager - Java Plugin Framework.
\end{itemize}
...
\subsection{Lost Password}
  \begin{sequencediagram}
    ...
    \end{call}
  \end{sequencediagram}
\subsection{External Authentication via OpenID}
...
\chapter{Publishing and Aggregation Platform}
	\section{Object Model}
      ...
      manipulation. Objects are not stored in the Publishing and Aggregation
      Platform, but are merely passed through its pipeline. 
		...
		\subsubsection{Service Associations}
			...
			This reflects the idea that an object, as 
		...
		\subsubsection{Properties}
			...
			belonging to the object in question. Child objects are not mandated to be of a unique type or value, and thus cannot be addressed by type or value. Instead, objects can be queried for child objects, given criteria about the child objects themselves.
	...
	\subsection{Sequence Diagrams}
      \emph{(Only showing sequences between object and service.)}
      ...
      \begin{call}{obj}{checkObject(this)}{req}{}
      \end{call}

      \begin{call}{obj}{push(this)}{svc}{}
        \begin{call}{svc}{addProperties(newProperties)}{obj}{}
        \end{call}
        \begin{call}{svc}{addAssoc(serviceAssoc)}{obj}{}
        \end{call}
      \end{call}

      \end{sequencediagram}
      \subsubsection{Pulling from a Service}
      \begin{sequencediagram}
      \newthread{obj}{Object}
      
      \newinst[1]{svc}{Service Request}

      \begin{call}{obj}{pull(this)}{svc}{}
        \begin{call}{svc}{addProperties(newProperties)}{obj}{}
        \end{call}
      \end{call}
	...
	\section{Server Design}
		...
		\subsection{Architecture}
			...
			HandlerChain class holds a linked list of handlers. As demonstrated below:
				\begin{figure}[h!] \centering \begin{sequencediagram}
...
of hard coding into the code what the next handler will be for each. 
				...
				\begin{figure}[h!] \centering \includegraphics{uml/HandlerChain.2} \end{figure}
				\newpage
		...
		\subsection{Handlers}
			\subsubsection{Handler}
				...
				\begin{figure}[h!] \centering \includegraphics{uml/Handlers.1} 
				\caption{Interface: Handler} \end{figure}
			...
			\subsubsection{ChainableHandler}
				...
				\begin{figure}[h!] \centering \includegraphics{uml/Handlers.2} 
				\caption{Class: ChainableHandler} \end{figure}
			...
			\subsubsection{AuthenticationHandler}
				...
				\begin{figure}[h!] \centering \includegraphics{uml/Handlers.3} 
				\caption{Class: AuthenticationHandler} \end{figure}
			...
			\subsubsection{SerializationHandler}
				...
				will take the Content object from the Context. Then, it will reserialize 
				it and place it in the Response.
				\begin{figure}[h!] \centering \includegraphics{uml/Handlers.4} 
				\caption{Class: SerializationHandler} \end{figure}
			...
			\begin{figure}[h!] \centering \includegraphics{uml/Handlers.5} 
			\caption{Class: RequiredDataVerificationHandler} \end{figure}
			...
			\begin{figure}[h!] \centering \includegraphics{uml/Handlers.6} 
			\caption{Class: ContentProcessingHandler} \end{figure}


			\subsubsection{Overall}

			\begin{figure}[h!] \centering \includegraphics{uml/Handlers.7} 
											\caption{Overall} \end{figure}
                        \clearpage
  ...
  \section{Sequence Diagram}
    ...
    \newinst{svc}{Service Plugin}
		...
		\begin{call}{req}{Authenticate}{auth}{}
			\begin{call}{auth}{Build Objects}{serial}{}
				\begin{call}{serial}{Verify Data}{ver}{}
					\begin{call}{ver}{Consult Plugin}{svc}{}
					\end{call}
					\begin{call}{ver}{Process the Request}{proc}{}
						\begin{call}{proc}{Run Request}{svc}{}
						\end{call}
          \end{call}
        \end{call}
      \end{call}
	...
	\section{Plugin Design}
		\subsection{Basic Design}
			The ContentProcessor expects to find a list of plugins available to it 
			which will be categorized by service, operation, and content type. It will 
			choose the plugins to used based on the content type of the incoming 
			request and then pass the content, operation, and content type to the 
			plugin. The plugin will then perform the appropriate action and return the 
			content with the expected data filled in. The lists of available plugins 
			will be maintained by a plugin manager will be built on top of a plugin 
			framework.

			\begin{figure}[h!]
				\centering
				\includegraphics[width=6in]{uml/PluginArchitecture.5}
				\caption{Plugin Architecture}
			\end{figure}
				\clearpage
		\subsection{ServicePlugin Interface}
			This is the interface that a Service Plugin provide in order to be used by 
			the Content Processing Handler. It provides two basic functionalities.  
			First, it must verify if the user provided by a user is adequate. Second, 
			it must be able to be able to accept information and then perform the 
			expected action.
			\begin{figure}[h!]
				\centering
				\includegraphics{uml/PluginArchitecture.1}
				\caption{Interface: ServicePlugin}
			\end{figure}	
...
\section{Disadvantages of Design}
\begin{itemize}
...
% LocalWords:  Mashbot OAuth OpenID
